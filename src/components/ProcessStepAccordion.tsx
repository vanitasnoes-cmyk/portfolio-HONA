...
function getStepBadges(text: string, evidence: StepEvidence, lang = 'vi') {
  const badges: { key: string; label: string; className: string; icon: 'image' | 'ai' | 'hitl' }[] = [];

  if (hasEvidence(evidence)) {
    badges.push({
      key: 'evidence',
      label: lang === 'en' ? 'Evidence Image' : 'Ảnh minh chứng',
  ...
  if (
    ...
    badges.push({
      key: 'prompt',
      label: lang === 'en' ? 'Prompt / AI Tools' : 'Prompt / Công cụ AI',
      className: 'bg-indigo-50 text-indigo-800 border-indigo-200/80 dark:bg-indigo-950/30 dark:text-indigo-400 dark:border-indigo-900/30',
      icon: 'ai',
    });
  }

  if (/kiểm chứng|hiệu đính|đối soát|ảo giác|thẩm định|rà soát|chỉnh sửa|verification|refinement|hallucination|audit|editing/i.test(text)) {
    badges.push({
      key: 'hitl',
  ...
  return (
    <div className="space-y-2">
      {steps.map((step, idx) => {
        ...
        const badges = getStepBadges(step.text, evidence, lang);
        ...
        return (
          ...
          >
            ...
            >
              <span className="w-7 h-7 rounded-lg bg-indigo-50 dark:bg-indigo-950/50 text-indigo-700 dark:text-indigo-400 text-[11px] font-black flex items-center justify-center shrink-0 border border-indigo-100/60 dark:border-indigo-900/40">
                {stepNum}
              </span>
                    ...
                    {urls.map((src, imgIdx) => (
                      ...
                      >
                        <img
                          ...
                          alt={
                            lang === 'en'
                              ? `Lesson ${lessonNumber} – Step ${stepNum}: ${title}${urls.length > 1 ? ` (${imgIdx + 1})` : ''}`
                              : `Bài ${lessonNumber} – Bước ${stepNum}: ${title}${urls.length > 1 ? ` (${imgIdx + 1})` : ''}`
                          }
                          loading="lazy"
